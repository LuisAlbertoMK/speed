...
export class EditaElementoComponent implements OnInit,OnChanges  {
  ...
  guardaInfo(){
    ...
    const {ok, faltante_s } = this._publicos.realizaValidaciones(['id','nombre','descripcion','tipo'],info)
      ...
      this._publicos.mensaje_pregunta(`Actualizar información de elmento`,true, `<strong> ${nombre} </strong>`).then(({respuesta})=>{
        if (respuesta) {
          ...
          update(ref(db), updates).then(()=>{
            this._publicos.swalToast('actualización correcta!!')
          })
          .catch(err=>{
            this._publicos.swalToastError('erro al actualizar!!')
          })
        }
      })
    }
